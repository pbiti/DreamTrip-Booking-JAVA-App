...
public class TodoDaoImpl implements TodoDao {
	...
	private static final String SELECT_TODO_BY_USERNAME = "select id,title,username,description,target_date,is_done from todos where username=?";
	...
	private static final String CHECK_FOR_UPDATE = "select * from todos where id=? and username=?";
	...
	public Todo selectTodo(long todoId, String username) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Todo(id, title, username1, description, targetDate, isDone);
	...
	/******************/
	public List<Todo> selectTodoByuname(String todousername) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			preparedStatement.setString(1, todousername);
			System.out.println(preparedStatement);
			// Step 3: Execute the query or update query
			ResultSet rs = preparedStatement.executeQuery();
	...
	/***********************/
	@Override
	public List<Todo> Todos_after_edit(Todo todo, String username) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Todo(id, title, username1, description, targetDate, isDone));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
